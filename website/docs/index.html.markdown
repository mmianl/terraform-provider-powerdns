...
- `cache_requests` - (Optional) Set this to `true` to enable cache of the PowerDNS REST API requests. This can also be specified with the `PDNS_CACHE_REQUESTS` environment variable. Changes made through this provider drop the cached copy of the affected zone, so they are visible to later reads in the same run. `WARNING! Enabling this option can still lead to the use of stale records when other automation populates the DNS zone records at the same time, since those changes are invisible to the cache until the TTL expires.`
